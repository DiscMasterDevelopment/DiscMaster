...
<div class="fieldcontain ${hasErrors(bean: artistInstance, field: 'description', 'error')} required">
    ...
    <g:select id="description" name="description.id" from="${discmaster.Description.list()}" optionKey="id" required=""
              value="${artistInstance?.description?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: artistInstance, field: 'news', 'error')} ">
    ...
    <g:select name="news" from="${discmaster.NewsArticle.list()}" multiple="multiple" optionKey="id" size="5"
              value="${artistInstance?.news*.id}" class="many-to-many"/>
    ...
    <ul class="one-to-many">
        ...
        <li class="add">
            <g:link controller="product" action="create"
                    params="['artist.id': artistInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'product.label', default: 'Product')])}</g:link>
        </li>
    </ul>
